...
const Todo = () => {
  ...
  const fetchTodos = async () => {
    try {
      const res = await axios.get("https://todo-app-xh5q.onrender.com")
      setTodos(res.data)
    } catch (err) {
      console.error(err)
    }
  ...
  const addTodo = async () => {
    if (input.trim()) {
      try {
        const res = await axios.post("https://todo-app-xh5q.onrender.com", {
          text: input
        })
               ...
               todos.map((todo) => (
                    ...
                    rounded-full bg-slate-100 border-0 bg-transparent"
                     >
                        ...
                        onChange={async () => {
                          await axios.put(`https://todo-app-xh5q.onrender.com/${todo["number of list"]}`, {
                        ...
                        <button onClick={async () => { await axios.delete(`https://todo-app-xh5q.onrender.com/${todo["number of list"]}`) 
                              fetchTodos()
